...
class ExampleTranslate(BaseModel):
    sub_id = models.CharField(max_length=100, blank=True, null=True, db_index=True) # uuid v7
    value = models.TextField(blank=True, null=True)
    word = models.ForeignKey('core.Word', on_delete=models.SET_NULL, blank=True, null=True, related_name='word_translated_examples') # word id - Cân nhắc dịch chuyển sang từ của người dùng thay vì official
    bold = models.JSONField(default=list, blank=True, null=True) #json index[][]..
    translate = models.ForeignKey('core.Translate', on_delete=models.SET_NULL, blank=True, null=True, related_name='translate_translated_examples') # word translate
    example = models.ForeignKey('core.Example', on_delete=models.SET_NULL, blank=True, null=True, related_name='translated_examples') # origin example
